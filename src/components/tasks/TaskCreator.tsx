...
export function TaskCreator({ onCreated, onCancel }: TaskCreatorProps) {
  ...
  const toggleTerm = (id: number, list: 'category_ids' | 'tag_ids') => {
    ...
    if (current.includes(id)) {
      setForm((f) => ({ ...f, [list]: (f[list] as number[]).filter((x) => x !== id) }));
    } else {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.prompt.trim()) { setError('Prompt / topic is required'); return; }
    if (!form.site_id) { setError('Please select a site'); return; }
    if (form.post_count < 1 || form.post_count > 100) { setError('Post count must be 1–100'); return; }
    if (form.interval_minutes < 1) { setError('Interval must be at least 1 minute'); return; }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-xs font-medium text-neutral-400">Prompt / Topic</label>
        <textarea
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-medium text-neutral-400">WordPress Site</label>
          <select
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-medium text-neutral-400">Post Type</label>
          <select
            className="bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2 text-sm text-neutral-100 focus:outline-none focus:ring-1 focus:ring-blue-500"
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-medium text-neutral-400">Number of Posts</label>
          <input
            type="number"
            min={1}
            max={100}
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-medium text-neutral-400">Interval (minutes)</label>
          <input
            type="number"
            min={1}
            className="bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2 text-sm text-neutral-100 focus:outline-none focus:ring-1 focus:ring-blue-500"
            value={form.interval_minutes}
            onChange={(e) => set('interval_minutes', parseInt(e.target.value) || 60)}
          />
        </div>
      </div>
      ...
      {categories.length > 0 && (
        <div className="flex flex-col gap-2">
          <label className="text-xs font-medium text-neutral-400">Categories</label>
        ...
        {showAdvanced && (
          <div className="px-4 pb-4 pt-2 space-y-4 border-t border-neutral-800">
            <Input
              ...
              onChange={(e) => set('model_override', e.target.value || undefined as any)}
              hint="Override the default AI model for this task"
            />
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-neutral-400">System Prompt Override</label>
              <textarea
                className="bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2 text-sm text-neutral-100 placeholder-neutral-600 focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none"
                rows={4}
                placeholder="Leave empty to use default from Settings..."
                value={form.system_prompt ?? ''}
                onChange={(e) => set('system_prompt', e.target.value || undefined as any)}
              />
              <p className="text-xs text-neutral-600">Overrides the global system prompt for this task only</p>
            </div>
          </div>
        )}
      </div>
