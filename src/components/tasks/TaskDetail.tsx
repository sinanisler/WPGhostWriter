...
export function TaskDetail({ taskId }: TaskDetailProps) {
  ...
  if (loading) {
    return <div className="text-neutral-500 text-sm py-8 text-center">Loading...</div>;
  }

  if (!data) {
    return <div className="text-red-400 text-sm py-8 text-center">Failed to load task</div>;
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-base font-semibold text-neutral-100">{task.name}</h3>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {(task.status === 'running' || task.status === 'paused') && (
            <Button size="sm" variant="danger" onClick={() => handleAction('cancel')}>Cancel</Button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: 'Posts', value: `${task.posts_completed}/${task.post_count}` },
          { label: 'Step', value: stepLabel(task.current_step) },
          { label: 'Cost', value: formatCost(task.total_estimated_cost) },
          { label: 'Tokens', value: String(task.total_prompt_tokens + task.total_completion_tokens) },
        ].map((s) => (
          <div key={s.label} className="bg-neutral-950 rounded-lg p-3">
            <p className="text-xs text-neutral-600">{s.label}</p>
            <p className="text-sm font-medium text-neutral-200 mt-0.5">{s.value}</p>
          </div>
        ))}
      </div>
      ...
      {/* Post table */}
      <div>
        <h4 className="text-xs font-semibold text-neutral-500 uppercase tracking-wide mb-2">Posts</h4>
            ...
            <tbody>
              {posts.map((post) => (
                <tr key={post.id} className="border-t border-neutral-800/50">
                  <td className="px-3 py-2 text-neutral-600">{post.sequence_number}</td>
                  <td className="px-3 py-2 text-neutral-300 max-w-xs">
                    <span className="truncate block">{post.title ?? '...'}</span>
                    {post.excerpt && (
                      <span className="text-neutral-600 text-xs truncate block">{post.excerpt}</span>
                    )}
                    {post.error_message && (
                      <span className="text-red-400 text-xs block">{post.error_message}</span>
                    )}
                  </td>
                  <td className="px-3 py-2">{statusBadge(post.status)}</td>
                  <td className="px-3 py-2 text-neutral-500">{formatCost(post.estimated_cost)}</td>
              ...
              {posts.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-3 py-6 text-center text-neutral-600">No posts yet</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      {/* Log feed */}
      <div>
        <h4 className="text-xs font-semibold text-neutral-500 uppercase tracking-wide mb-2">Activity Log</h4>
        <div className="bg-neutral-950 border border-neutral-800 rounded-lg p-3 h-48 overflow-y-auto font-mono text-xs space-y-0.5">
          {logs.length === 0 && (
            <div className="text-neutral-700 py-2 text-center">No log entries yet</div>
          )}
          {logs.map((log) => (
            <div key={log.id} className="flex gap-2">
              <span className="text-neutral-700 shrink-0">{log.created_at.split('T')[1]?.split('.')[0] ?? ''}</span>
