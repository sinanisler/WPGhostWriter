      ...
      <div className="flex-1 p-6 overflow-y-auto">
        {tasks.length === 0 ? (
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-12 text-center">
            <p className="text-neutral-500 mb-4">No tasks yet. Create your first content automation task.</p>
            <Button onClick={() => setShowCreator(true)}>Create Task</Button>
          </div>
        ) : (
          <div className="space-y-3">
            {tasks.map((task) => {
              const progress = task.post_count > 0
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    >
                      ...
                      <p className="text-xs text-neutral-500 truncate">{task.prompt}</p>
                      <div className="flex items-center gap-4 mt-2 text-xs text-neutral-600">
                        <span>{task.post_count} posts · {task.posts_completed} done</span>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {task.status === 'pending' && (
                        <Button size="sm" onClick={() => handleStart(task.id)}>Start</Button>
                      )}
                      {task.status === 'running' && (
                        <Button size="sm" variant="secondary" onClick={() => handlePause(task.id)}>Pause</Button>
                      )}
                      {task.status === 'paused' && (
                        <Button size="sm" onClick={() => handleResume(task.id)}>Resume</Button>
                      )}
                      {(task.status === 'running' || task.status === 'paused') && (
                        <Button size="sm" variant="danger" onClick={() => handleCancel(task.id)}>Cancel</Button>
                      )}
                      <Button
                        size="sm"
                        variant="ghost"
                      ...
                      {(task.status === 'completed' || task.status === 'failed' || task.status === 'cancelled' || task.status === 'pending') && (
